...
{
    if (this->mesh != nullptr && this->quadData != nullptr && this->visible) {
        glPushAttrib(GL_ALL_ATTRIB_BITS);

        glDepthRange(0.0,0.999999);
        glLineWidth(4);

        for (const int& pId : quadData->labels) {
            const QuadBoolean::QuadPatch& patch = quadData->patches[pId];
